...
function Arcs() {
  ...
  const items = koros.arcs().map((a) => {
    const project = koros.project(a.project) ?? a.project;
    const own = project === a.title;
    return {
      id: a.slug,
      label: a.title,
      content: (
        <span className={styles.row} title={own ? a.title : `${a.title} · ${project}`}>
          {a.title}
          {!own && <span className={`caption ${styles.rowProject}`}>{` · ${project}`}</span>}
        </span>
      ),
      suffix: a.needsYou ? (
