...
import { createLocalSource, createWorkbench, helloPlugin } from '../host';
...
describe('Workbench', () => {
  it('shows the pinned navigator and opens a document from it', async () => {
    ...
    await user.click(within(sidebar).getByRole('button', { name: /say hello to alpha/i }));
  ...
  it('opening the same document again focuses it instead of duplicating', async () => {
    ...
    const open = (name: string) =>
      user.click(screen.getByRole('button', { name: `Say hello to ${name}` }));
    await open('alpha');
    await open('beta');
    await open('alpha');
  ...
  it('closes the focused panel from the keyboard and announces it', async () => {
    const user = userEvent.setup();
    mount();
    await user.click(screen.getByRole('button', { name: 'Say hello to alpha' }));
  ...
  it('splits and moves focus by keyboard', async () => {
    ...
    mount();
    await user.click(screen.getByRole('button', { name: 'Say hello to alpha' }));
    await user.click(screen.getByRole('button', { name: 'Say hello to beta' }));
  ...
  it('restores the layout from storage on the next mount', async () => {
    ...
    await user.click(screen.getByRole('button', { name: 'Say hello to gamma' }));
  ...
  it('keeps the rest interactive when a panel crashes', async () => {
    ...
    mount();
    await user.click(screen.getByRole('button', { name: 'Say hello to alpha' }));
    await user.click(screen.getByRole('button', { name: 'Say hello to crash' }));
